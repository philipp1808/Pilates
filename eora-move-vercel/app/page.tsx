...
export default function Home() {
  ...
  return <main>
    ...
    <section className="home-classes">
      ...
      <div className="home-class-grid">{courses.slice(0, 3).map(course => <a className="home-class-card" href="/angebot" key={course.name}><div className={course.tone} aria-hidden="true"><span>{course.strap}</span><b>{course.number}</b><div className="visual-line" /></div><div><p>{course.number} / {course.level}</p><h3>{course.name}</h3><span>{course.description}</span><strong>Class entdecken <ArrowUpRight /></strong></div></a>)}</div>
      <a className="button home-outline-button" href="/angebot">Alle Kurse ansehen <ArrowUpRight /></a>
    </section>

    <section className="home-signature">
      <div className="brand-image why-eora-image" role="img" aria-label="Frau beim Reformer-Training im Eora Move Studio" />
      <div><p className="eyebrow">Why Eora</p><h2>Small groups.<br/>Real attention.</h2><ul><li><span>01</span> Persönliche Korrekturen</li><li><span>02</span> Durchdachte Trainingsmethoden</li><li><span>03</span> Ein Studio zum Ankommen</li></ul><a className="button button-primary" href="/ueber-uns">Mehr über uns <ArrowUpRight /></a></div>
    </section>

    <section className="home-starter">
      <div><p className="section-index">03 / Dein Einstieg</p><p className="eyebrow">New here</p><h2>Your first<br/><em>move.</em></h2><p>Starte mit einer einzelnen Class oder entdecke Eora in drei Sessions.</p></div>
      <article className="starter-card"><p>Welcome Trio</p><h3>3 Trainings</h3><dl><div><dt>Gültigkeit</dt><dd>30 Tage</dd></div><div><dt>Preis</dt><dd>49 €</dd></div></dl><button onClick={() => openBooking('Welcome Trio')}>Paket auswählen <ArrowUpRight /></button></article>
      <a className="dark-link" href="/preise">Alle Preise vergleichen <ArrowUpRight /></a>
    </section>
